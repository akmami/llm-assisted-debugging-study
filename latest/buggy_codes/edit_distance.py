# Inputs where the cheapest-looking path contains adjacent swaps will be underestimated. For example, transforming 'ab' to 'ba' should require two substitutions or a delete plus insert, but the buggy program returns 1 by treating the swap as a single operation. Longer strings with local adjacent transpositions, such as 'abcd' versus 'acbd', or strings with repeated swapped patterns, will also fail. Cases without useful adjacent transpositions will often still pass.
'''
