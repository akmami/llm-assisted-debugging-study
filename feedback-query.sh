...
run_participants_answers() {
    shopt -s nullglob
    mkdir -p work/llm_inputs

    for dir in "$USER_DIR"/work/participant_packets/*/ ; do
        dir=${dir%/};
        base=$(basename "$dir");
        ...
        mkdir -p work/user_responses/"$USER_ID";
        OUTFILE="work/user_responses/$USER_ID/${PROJ}_${BUG}.response.txt";
...
if [ -d "$USER_DIR" ]; then
    if [[ -d "$USER_DIR/work" && -d "$USER_DIR/work/participant_packets" ]]; then
        run_participants_answers;
    else
        echo "$USER_DIR/work/participant_packets directory does not exist ...";
    fi
else
